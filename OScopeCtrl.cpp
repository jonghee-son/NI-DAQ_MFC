...
COScopeCtrl::COScopeCtrl()
{
  ...
  m_strXUnitsString.Format(_T("")) ;  // can also be set with SetXUnits
...
void COScopeCtrl::DrawPoint()
{
  ...
  if (m_dcPlot.GetSafeHdc() != NULL)
  {
    ...
    m_dcPlot.BitBlt(m_rectPlot.left+m_nShiftPixels, m_rectPlot.top+1, 
    ...
    rectCleanUp.right  = rectCleanUp.left+m_nShiftPixels ;
	// fill the cleanup area with the background
    m_dcPlot.FillRect(rectCleanUp, &m_brushBack) ;

	rectCleanUp = m_rectPlot ;
    rectCleanUp.left  = rectCleanUp.right;
	rectCleanUp.left  = rectCleanUp.left+m_nShiftPixels;
