...
public:
	...
	void SetFormatString(CString strFormat=_T("%.3d"))	{ 
	...
	void DisplayInt(int nValue, bool bSetFmtStr=false, CString strFormat=_T("%.3d"))	{ 
		m_bFloat=false;
		if (bSetFmtStr) m_strFormat = strFormat;
		m_strDisplay.Format(m_strFormat, nValue); 
		Invalidate(FALSE); 
	};
	void DisplayFloat(float fValue, bool bSetFmtStr=false, CString strFormat=_T("%.2f"))	{
		m_bFloat=true; 
		if (bSetFmtStr) m_strFormat = strFormat;	
		m_strDisplay.Format(m_strFormat, fValue); 
		Update();	
	};
	void DisplayTime( CString strFormat = _T("%H:%M:%S")){
		m_bDraw3DBar=false; Display( (CTime::GetCurrentTime()).Format(strFormat)); };
	void DisplayTime(UINT uSecs, UINT uMins=0, UINT uHours=0, UINT uDays=0, CString strFormat=_T("%D:%H:%M:%S")) {	
